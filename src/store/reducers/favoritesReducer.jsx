...
const favoritesReducer = (state = initState, action) => {
    ...
    switch (action.type) {
        case ADD_FAVORITES_PUBLICATION:
            return {...state, publications: [...state.publications],user: action.payload.userId ,loading: false, error: ''}
        case LOAD_FAVORITES_PUBLICATION:
            return {...state.publications, publications: action.payload.publications,user: action.payload.userId ,loading: false, error: ''}
        case IS_FAVORITE:
